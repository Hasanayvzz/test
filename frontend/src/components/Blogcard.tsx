...
import SVGFavoriteBlogs from "./SVGFavorite";
...
const BlogCard = ({ blogData, favorites, trigger, setTrigger }) => {
  ...
  return (
    <>
      {!isLoading ? (
        blogData
          .filter((blog) => blog.status === "Approved") // status değeri true olan blogları filtreliyoruz
          .map((blog) => (
            <div className={`project-card-container  ${theme.theme}`}>
              <div className="d-flex justify-content-end align-items-end">
                <div
                  onClick={() =>
                    user
                      ? handleAddFavorite(blog._id, user)
                ...
                >
                  <SVGFavoriteBlogs
                    fill={
                      favorites && favorites.includes(blog._id) ? "#d9436d" : ""
                    }
                  ></SVGFavoriteBlogs>
                </div>
              </div>
